...
float 
STM32Encoder::getAngle(unsigned char base){
...
unsigned char 
STM32Encoder::getDirection(){
    return *bb_perip(&(_TIMER->regs).gen->CR1, TIMER_CR1_DIR_BIT);
}

void 
STM32Encoder::Polarity(unsigned char pol) {
    if ( pol == STM32E_INVERTED) {
...
void 
STM32Encoder::setPrescaler(unsigned int prescale){
...
unsigned int 
STM32Encoder::getPrescaler(){
    return timer_get_prescaler(_TIMER); 
};

void 
STM32Encoder::setPPR(unsigned int pulserPerRevolution){
...
void 
STM32Encoder::attachInterrupt(voidFuncPtr func) {
...
void 
STM32Encoder::setFilter(unsigned char val) {
...
/*
   ...
   if (Direction() == STM32E_POSITIVE) _revolutions++;
   if (Direction() == STM32E_NEGATIVE) _revolutions--;
   }*/
...
void 
STM32Encoder::remapInputs(unsigned char remap_type) { 
    afio_init(); 
    ...
    switch(_TIMER) {
        ...
        case TIMER4:
            //hmmm... bit 12
            shift = 12;
            remap_type = remap_type % 1; //this is a bit, so we can only shift 1 or 0, not 3.
            break;
            //TO DO: Support for Maple Native. 
    } //end switch
    afio_remap( (remap_type << shift));
}
